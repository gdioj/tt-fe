...
export function DataTable<TData, TValue>({
  ...
  const getColumnType = (columnId: string): ColumnType => {
    ...
    if (columnId.includes('rate') || columnId.includes('amount') || columnId.includes('price') || columnId.includes('salary')) {
      return 'number';
    }
    return 'text';
  };

  // Get filterable columns (exclude action columns)
  const filterableColumns: FilterableColumn[] = columns.filter(column => {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const columnDef = column as any;
    return (columnDef.accessorKey || column.id) && typeof column.header === 'string';
  }).map(column => {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const columnDef = column as any;
  ...
  const addFilter = () => {
    ...
    const existingFilterIndex = appliedFilters.findIndex(
      filter => filter.id === selectedColumn && filter.value === filterValueToAdd
    );
    ...
    if (existingFilterIndex === -1) {
      ...
      const existingColumnFilterIndex = updatedColumnFilters.findIndex(f => f.id === selectedColumn);
  ...
  const removeFilter = (filterToRemove: ColumnFilter) => {
    ...
    // Update react-table column filters
    const updatedColumnFilters = [...columnFilters];
    const columnFilterIndex = updatedColumnFilters.findIndex(f => f.id === filterToRemove.id);
      ...
      case 'number':
        return (
          <NumberRange
            min={numberRange.min}
            max={numberRange.max}
            onRangeChange={(min?: number, max?: number) => setNumberRange({ min, max })}
            disabled={!selectedColumn}
  ...
  const renderMobileCard = (row: Row<TData>) => {
    // Get all visible cells from the row
    ...
    const dataCells = visibleCells.filter(cell => {
      ...
      return (columnDef.accessorKey || cell.column.id) && typeof cell.column.columnDef.header === 'string';
    });
        ...
        <CardContent className="p-4">
          <div className="space-y-2">
            {dataCells.map((cell, index) => {
              ...
              return (
                <div key={cell.id} className={index === 0 ? "font-semibold text-base" : "text-sm"}>
                  {index === 0 ? (
  ...
  const table = useReactTable({
    ...
    filterFns: {
      ...
      dateFilter: (row: any, columnId: string, filterValue: string) => {
        ...
        const cellValue = row.original[columnId as keyof typeof row.original] as string;
      ...
      numberRangeFilter: (row: any, columnId: string, filterValue: string) => {
        // Get the raw unformatted value from the row's original data
        const cellValue = row.original[columnId as keyof typeof row.original] as number;
        const [min, max] = filterValue.split(':').map((v: string) => v === '' ? undefined : parseFloat(v));
      ...
      {appliedFilters.length > 0 && (
        <div className="flex items-center gap-2 flex-wrap">
          ...
          {appliedFilters.map((filter, index) => (
            <Badge key={`${filter.id}-${filter.value}-${index}`} variant="secondary" className="flex items-center gap-1 cursor-pointer hover:bg-secondary/80"
            ...
            <TableBody>
              {table.getRowModel().rows.length ? (
                table.getRowModel().rows.map((row) => (
                  <TableRow
                    key={row.id}
                    className={`hover:bg-muted/70 hover:shadow-sm transition-all duration-200 ease-in-out group ${onRowClick !== undefined ? 'cursor-pointer' : ''}`}
                    onClick={() => handleRowClick(row)}
                  >
                    {row.getVisibleCells().map((cell) => (
                      <TableCell key={cell.id} className="font-medium group-hover:text-foreground/90">
                        {flexRender(cell.column.columnDef.cell, cell.getContext())}
                      </TableCell>
                    ))}
                  </TableRow>
